...
#STRUCTURE OF DATABASE (in .csv):
 # | YEAR | SERIAL | HHWT | METAREA | OWNERSHIP | RENTGRS | HHINCOME | VALUEH |
...
home_dfs_by_year <- lapply(home_dfs_by_year, function (x) x[!duplicated(x) & 
                                                            x$HHINCOME > 0 & 
                                                            complete.cases(x),])
...
remove_missing_VALUEHs <- function(x) {
  subset(x, x$VALUEH != 9999999 & x$VALUEH != 9999998 & x$VALUEH != 0000000)
}

#remove housholds with OWNERSHIP as 0 as in NA
remove_missing_OWNERSHIP <- function(x) {
  subset(x, x$OWNERSHIP != 0)
}

#MASTER FUNCTION TO CALCULATE 10th, 90th PERCENTILES AND VARIANCE
tenth_ninetieth_percentile_by_year_by_msas <- function(year, value = "VALUEH", ownership = 1, type = "percentile") {
  year_by_msas <- split(year, year$METAREA)
  if (type == "percentile") {
    lapply(year_by_msas, function(msa) quantile(subset(msa, msa$OWNERSHIP == ownership)[[value]], 
                                                probs = c(0.10,0.90)))
  } else if (type == "variance"){
    lapply(year_by_msas, function(msa) var(subset(msa, msa$OWNERSHIP == ownership)[[value]]))
  }
} 
...
percentile_10th_90th_VALUEH_own_dwelling <- lapply(home_dfs_values_cleaned, 
                                                   tenth_ninetieth_percentile_by_year_by_msas, 
